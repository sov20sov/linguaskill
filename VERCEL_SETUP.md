...
- **Rate Limit:** نقطة `/api/contact` محمية بحد طلبات (5 طلبات كل 15 دقيقة لكل IP) لتقليل الإساءة والسبام.
- **رؤوس أمان:** الدالة تضيف رؤوس مثل `X-Content-Type-Options` و `X-Frame-Options` و `X-XSS-Protection`.
- **الأسرار:** لا تُعرّف أي مفاتيح سرية (مثل `RESEND_API_KEY` أو `GEMINI_API_KEY`) في الـ Frontend أو في `vite.config.ts` — تُستخدم فقط على الخادم أو في Environment Variables في Vercel.
